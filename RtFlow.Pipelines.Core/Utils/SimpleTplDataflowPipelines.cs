...
namespace RtFlow.Pipelines.Core.Utils;
/// <summary>
...
public readonly struct PipelineBuilder<TInput>
{
    ...
    /// </remarks>
    public readonly PipelineBuilder<TInput> AddUnlinked<TNewInput>(ITargetBlock<TNewInput> block)
    {
        ArgumentNullException.ThrowIfNull(block);
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput, TNewOutput> AddUnlinked<TNewInput, TNewOutput>(
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput> WithPostCompletionAction(Action<Task> action)
    {
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput> WithPostCompletionAction(Func<Task, Task> action)
    {
        ArgumentNullException.ThrowIfNull(action);
    ...
    /// </remarks>
    public readonly ITargetBlock<TInput> ToPipeline()
    {
        ...
        var completion = PipelineCommon.CreatePipelineAsync(_target, newNodes);
...
public readonly struct PipelineBuilder<TInput, TOutput>
{
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput> LinkTo(ITargetBlock<TOutput> block)
    {
        ArgumentNullException.ThrowIfNull(block);
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput, TNewOutput> LinkTo<TNewOutput>(
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput> AddUnlinked<TNewInput>(
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput, TOutput> WithPostCompletionAction(Action<Task> action)
    {
        ArgumentNullException.ThrowIfNull(action);
        var newNode = new ActionNode(t => { action(t); return Task.CompletedTask; });
    ...
    /// The current builder is not changed.
    /// </remarks>
    public readonly PipelineBuilder<TInput, TOutput> WithPostCompletionAction(Func<Task, Task> action)
    {
        ArgumentNullException.ThrowIfNull(action);
        var newNode = new ActionNode(action);
        return new PipelineBuilder<TInput, TOutput>(_target, _source, _nodes, newNode);
    ...
    /// are forcefully completed and their output is discarded.
    /// </remarks>
    public readonly IPropagatorBlock<TInput, TOutput> ToPipeline()
    {
        if (_target == null) throw new InvalidOperationException();
...
internal sealed class LinkNode<TOutput> : Node
{
    ...
    public override void Fault()
    {
        ...
        // 1. Divert any future output immediately.
        _blockAsSource?.LinkTo(
                DataflowBlock.NullTarget<TOutput>(),
                _nullTargetLinkOptions);

        // 2. Now force-complete the block.
        try
        {
            _block.Fault(new PipelineException());
        }
        catch (InvalidOperationException) { /* block raced to Complete/Fault */ }
    }
}
...
{
    internal static Task CreatePipelineAsync<TInput>(ITargetBlock<TInput> target,
        Node[] nodes)
    {
        Debug.Assert(target != null);
        ...
        void onError() => errorTcs.TrySetResult(null);
        ...
        Debug.Assert(lastCompletion == null); // The final node has no target
